#!/usr/bin/env python3
# Copyright (c) 2018 The Qubnet Core developers
...
def createRefundTx(node, rawtx, script, lockTime, addrRefundFrom, addrRefundTo):
    ...
    addrRefundFromInfo = node.validateaddress(addrRefundFrom)
...
def createClaimTx(node, rawtx, script, lockTime, secret, addrClaimFrom, addrClaimTo):
    ...
    addrClaimFromInfo = node.validateaddress(addrClaimFrom)
...
class AtomicSwapTest(QubnetTestFramework):
    ...
    def run_test(self):
        ...
        ro = nodes[0].signrawtransaction(rawtxInitiate)
        ...
        ro = nodes[1].signrawtransaction(rawtx_p)
        assert(ro['complete'] == True)
        rawtx_p = ro['hex']
