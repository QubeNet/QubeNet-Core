// Copyright (c) 2017-2018 The Qubnet Core developers
...
int ExtractBip32InfoV(const std::vector<uint8_t> &vchKey, UniValue &keyInfo, std::string &sError)
{
    ...
    {
        keyInfo.pushKV("type", "Qubnet extended secret key");
    } else
...
int ExtractBip32InfoP(const std::vector<uint8_t> &vchKey, UniValue &keyInfo, std::string &sError)
{
    ...
    {
        keyInfo.pushKV("type", "Qubnet extended public key");
    } else
    ...
    if (mode == "import")
    {
        ...
        } else
        {
            if (!eKey58.IsValid(CChainParams::EXT_SECRET_KEY)
                ...
                throw std::runtime_error("Import failed - Key must begin with a qubnet prefix.");
        };
...
UniValue extkeyaltversion(const JSONRPCRequest &request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "If the provided ext_key has a Bitcoin prefix the output will be encoded with a Qubnet prefix.\n"
            "If the provided ext_key has a Qubnet prefix the output will be encoded with a Bitcoin prefix.");
...
UniValue getnewextaddress(const JSONRPCRequest &request)
{
    ...
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "Returns a new Qubnet ext address for receiving payments.\n"
            + HelpRequiringPassphrase(pwallet) +
            "\nArguments:\n"
            ...
            "\nResult:\n"
            "\"address\"              (string) The new qubnet extended address\n"
            "\nExamples:\n"
...
UniValue getnewstealthaddress(const JSONRPCRequest &request)
{
    ...
    if (request.fHelp || request.params.size() > 5)
        throw std::runtime_error(
            ...
            "Returns a new Qubnet stealth address for receiving payments."
            + HelpRequiringPassphrase(pwallet) +
            "\nArguments:\n"
            "1. \"label\"             (string, optional) If specified the key is added to the address book.\n"
            ...
            "\nResult:\n"
            "\"address\"              (string) The new qubnet stealth address\n"
            "\nExamples:\n"
...
UniValue manageaddressbook(const JSONRPCRequest &request)
{
    ...
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_PARAMETER, _("Invalid Qubnet address."));
...
UniValue getstakinginfo(const JSONRPCRequest &request)
{
    ...
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"moneysupply\": xxxxxxx,        (numeric) the total amount of qubnet in the network\n"
            "  \"reserve\": xxxxxxx,            (numeric) the total amount of qubnet in the network\n"
...
UniValue getcoldstakinginfo(const JSONRPCRequest &request)
{
    ...
    int nRequiredDepth = std::min((int)(Params().GetStakeMinConfirmations()-1), (int)(nHeight / 160));
    ...
    if (request.params.size() > 2 && !request.params[2].isNull()) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!address.IsValidStealthAddress())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Qubnet stealth address: ")+input.get_str());
    ...
    if (request.params[0].isArray())
    {
        ...
        for (size_t k = 0; k < outputs.size(); ++k)
        {
            ...
            if (typeOut == OUTPUT_RINGCT
                ...
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Qubnet stealth address");
...
UniValue sendtypeto(const JSONRPCRequest &request)
{
    ...
    if (request.fHelp || request.params.size() < 3 || request.params.size() > 9)
        throw std::runtime_error(
            ...
            "    3.1 \"address\"    (string, required) The qubnet address to send to.\n"
