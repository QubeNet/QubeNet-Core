...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet *wallet, const CWalletTx &wtx)
{
    ...
    } else
    {
        ...
        if (fAllFromMe && fAllToMe)
        {
            ...
            b3.append(TransactionRecord(hash, nTime, TransactionRecord::SendToSelf, "",
                            -(nDebit - nChange), nCredit - nChange));
            b3.last().involvesWatchAddress = involvesWatchAddress;   // maybe pass to TransactionRecord as constructor argument
        } else
        if (fAllFromMe)
        {
            //
            ...
            for (unsigned int nOut = 0; nOut < wtx.tx->vpout.size(); nOut++)
            {
                ...
                {
                    // Ignore b3 sent to self, as this is usually the change
        ...
        else
        {
            ...
            //
            b3.append(TransactionRecord(hash, nTime, TransactionRecord::Other, "", nNet, 0));
            b3.last().involvesWatchAddress = involvesWatchAddress;
        }
    }
