...
int64_t CChainParams::GetCoinYearReward(int64_t nTime) const
{
    ...
    static const int64_t nSecondsInMonth = 30 * 24 * 60 * 60;



    int64_t nYearsSinceGenesis = (nTime - genesis.nTime) / nSecondsInYear;

    int64_t nMonthsSinceGenesis = (nTime - genesis.nTime) / nSecondsInMonth;

    if(nMonthsSinceGenesis >= 0 && nMonthsSinceGenesis < 2)
        return (5000 ) * CENT;

    if(nMonthsSinceGenesis >= 2 && nMonthsSinceGenesis < 6)
        return (500 - nMonthsSinceGenesis*100) * CENT;


    if (nYearsSinceGenesis >= 0 && nYearsSinceGenesis < 5)
        return (50 - 10*nYearsSinceGenesis) * CENT;
...
int64_t CChainParams::GetProofOfStakeReward(const CBlockIndex *pindexPrev, int64_t nFees) const
{
    ...
    //If max supply reached, then only transaction fee will be transmited.
...
const std::pair<const char*, CAmount> genesisOutputs[] = {
    std::make_pair("81A12E682B074EC9AC643BAB64537F771E90C541", 5000 * COIN),
    std::make_pair("17586AA3DF7A66812E969324FD025915A62847AC", 5000 * COIN),
    std::make_pair("5BDA83F1E89415115A2008F4CD254737073B500E", 5000 * COIN),
    std::make_pair("5BDA83F1E89415115A2008F4CD254737073B500E", 5000 * COIN),
    std::make_pair("7663DA231864B3C2938E0F7A557FBA5ADC53F1D6", 2500 * COIN),
    std::make_pair("7663DA231864B3C2938E0F7A557FBA5ADC53F1D6", 2500 * COIN),
    std::make_pair("7663DA231864B3C2938E0F7A557FBA5ADC53F1D6", 2500 * COIN),
    std::make_pair("03925797A86A5CD9C083820403BE785D26E622B3", 2500 * COIN),
    std::make_pair("055E3DC048C68824A45102F4836D80428AFFAA53", 2000 * COIN),
    std::make_pair("23A1586B1499A0C8026CDA738605B8001A65EB79", 2000 * COIN),
    std::make_pair("0678A29CB8621B8CE35125BDD898530EDD436A87", 2000 * COIN),
    std::make_pair("57C0362BFEC3D28C5E3DC9AFC4CB0628AD906FA0", 2000 * COIN),
    std::make_pair("59547A7A2274DC69466A9657E0381B9950763C5D", 2000 * COIN),
    std::make_pair("70A67DEDF3A812170F2C84AF416479BDC09E8266", 10000 * COIN),
    std::make_pair("622C869FCC90FFA2A66802D95921D56749A022B9", 10000 * COIN),
    std::make_pair("A26027CF5A0CA1CE57E88D9B1CA4220D05F42DD0", 10000 * COIN),
    std::make_pair("AD4A902B35D742D1577F240A85CAEEE71D18550C", 10000 * COIN),
    std::make_pair("1393D487ACCAF41FD6187B4017111011973A6D86", 1000 * COIN),
    std::make_pair("34753DC2B3D9B7D3C93012AD7E4F8B0B8E045DDE", 1000 * COIN),
    std::make_pair("97D36868FD6D00B36637A5AE2F54A9A643768679", 1000 * COIN),
    std::make_pair("41B6FBD280818F65CD62F216FF204905B592A568", 1000 * COIN),
    std::make_pair("8F2F99B357A69DB7A340F50FA633C2C44DAB7720", 1000 * COIN),
    std::make_pair("F6C53B4E05356F5014E5DF8BFCF83EE090EF5770", 1000 * COIN),
    std::make_pair("E6EF292E0C476722BB8FB9253815F2DA52F8443D", 1000 * COIN),
    std::make_pair("DD38071EC25A0EF57071EECA49D9578CC429E944", 1000 * COIN),//
    std::make_pair("9BA030BAE37F99D863E061FD8E35923CDD7A58BB", 1000 * COIN),
    std::make_pair("7170BEE6D9A38AA8C6C5BA5A06F45F4DB9FEE4C6", 1000 * COIN),//
    std::make_pair("092327D1A6AED1B4E1138CAB952B40946235E8E1", 1000 * COIN),
    std::make_pair("B06CC793059883C469B82DEE4425E92677DF837D", 1000 * COIN),//
    std::make_pair("86209F08D9E887BACBB00178F6907CA9BDAB88F2", 1000 * COIN),
    std::make_pair("A450D03DCEBF5F97FCFA37DB3F092BCA76630DEB", 1000 * COIN),
    std::make_pair("D8A14894BB401271D51776CA3FB0697368FBCA6E", 1000 * COIN),
    std::make_pair("B61C7504FBE56B1A77B1393BF44123567E5EF973", 1000 * COIN),
    std::make_pair("AB8A9B65553B583E5B26E46DC0A7CC9A9B9ED851", 1000 * COIN),
    std::make_pair("45EBD0E9113CE3FE9E0E9EC6DBF94A5023291E3F", 1000 * COIN),
    std::make_pair("D97BB11F1F2682E809542B4E3D089D34FDF12FF4", 1000 * COIN),
    std::make_pair("5EB531D578548761AA0CE0BC58D56F994774CE86", 200 * COIN),
    std::make_pair("F5D1806C85D34E4DE4229FD1975C852C6C901783", 200 * COIN),
    std::make_pair("AD9A834F4B4E92DCD6FDD5C6A2F81364A51D3FDB", 200 * COIN),
    std::make_pair("22B6B5B0CF7EC1C1443FAFB0FFD3B274B7DA2377", 200 * COIN),
    std::make_pair("DB1075A2664B9ED631F94E941963C8E5BEC6BE91", 200 * COIN),
};
...
const std::pair<const char*, CAmount> genesisOutputsTestnet[] = {
    std::make_pair("A0A8739966475450A050324632AA4BC3648B5AB2",701708 * COIN),
    std::make_pair("A0A8739966475450A050324632AA4BC3648B5AB2",701708 * COIN),
    std::make_pair("A0A8739966475450A050324632AA4BC3648B5AB2",701708 * COIN),
};
...
static CBlock CreateGenesisBlockRegTest(uint32_t nTime, uint32_t nNonce, uint32_t nBits)
{
    ...
    genesis.nBits    = nBits;
    genesis.nNonce   = nNonce;
    genesis.nVersion = QUBNET_BLOCK_VERSION;
    genesis.vtx.push_back(MakeTransactionRef(std::move(txNew)));
...
{
    const char *pszTimestamp = "High school denies valedictorian a graduation speech, no reason given";
...
public:
    CMainParams() {
        ...
        nStakeMinConfirmations = 30;   // 30 * 2 minutes
        ...
        nPruneAfterHeight = 100000;
         ...
         do {
               a = a + 1;
               genesis = CreateGenesisBlockMainNet(1530957658, a, 0x1f00ffff);
        ...
        genesis = CreateGenesisBlockMainNet(1530957658, 29297, 0x1f00ffff);
        consensus.hashGenesisBlock = genesis.GetHash();

        assert(consensus.hashGenesisBlock == uint256S("0x0000855d5206f93909c7888eac8111a1cf60746f27bc436c625025f7886030a6"));
        assert(genesis.hashMerkleRoot == uint256S("0xb2cf55662d062637c32437ba03ce90c6945e28b953c48f34cc5d15ebbc5335d4"));
        assert(genesis.hashWitnessMerkleRoot == uint256S("0xd9f7cf11aa4c43a2bbff5f99828e98ea0a6f97d01d8e0c8e1a065956b13499fe"));
        ...
        vSeeds.emplace_back("13.236.157.150");




                vDevFundSettings.emplace_back(0,
                                              DevFundSettings("QgaHowGeZ73z4AWcFCV2WCD7sARNYk4c3V", 10, 1));
                vDevFundSettings.emplace_back(consensus.OpIsCoinstakeTime,
                                              DevFundSettings("QgaHowGeZ73z4AWcFCV2WCD7sARNYk4c3V", 10, 60));


                base58Prefixes[PUBKEY_ADDRESS]     = {0x3a};
                base58Prefixes[SCRIPT_ADDRESS]     = {0x55};
                base58Prefixes[PUBKEY_ADDRESS_256] = {0x39};
                base58Prefixes[SCRIPT_ADDRESS_256] = {0x3d};
                base58Prefixes[SECRET_KEY]         = {0x99};
...
public:
    CTestNetParams() {
        ...
        genesis = CreateGenesisBlockTestNet(1530957658, 41808, 0x1f00ffff);
        consensus.hashGenesisBlock = genesis.GetHash();

        assert(consensus.hashGenesisBlock == uint256S("0x0000ad0d25c8c542f4c559c1de1970f23f7d2cd457fa5e488a436afe55707d28"));
        assert(genesis.hashMerkleRoot == uint256S("0x2c4eba449e6f4bd1b8200a1d200768cabdd00d5ac697f017b0587e889dad5912"));
        assert(genesis.hashWitnessMerkleRoot == uint256S("0x8473070cc8efa7033106aab58824453574584e82bdd15bad007bdc722986ea8b"));

        vFixedSeeds.clear();
        vSeeds.clear();
        // nodes with support for servicebits filtering should be at the top
        vSeeds.emplace_back("testnet-seed.qubnet.io");
        vSeeds.emplace_back("dnsseed-testnet.qubnet.io");
