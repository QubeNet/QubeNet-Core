...
#if defined(NDEBUG)
# error "Qubnet cannot be compiled without assertions."
#endif
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (!fQubnetMode  // genesis coinbase is spendable when in Qubnet mode
        && fIsGenesisBlock) {
        if (!fJustCheck)
    ...
    blockundo.vtxundo.reserve(block.vtx.size() - (fQubnetMode ? 0 : 1));
    ...
    } else
    {
        ...
        if (block.vtx[0]->GetValueOut() > blockReward) // qubnet coins are imported as coinbase txns
...
void UpdateTip(const CBlockIndex *pindexNew, const CChainParams& chainParams) {
    ...
    if (!IsInitialBlockDownload())
    {
        ...
        for (int i = 0; i < 100 && pindex != nullptr; i++)
        {
            if (fQubnetMode)
            {
                if (pindex->nVersion > QUBNET_BLOCK_VERSION)
                    ++nUpgraded;
            } else
            {
...
unsigned int GetNextTargetRequired(const CBlockIndex *pindexLast)
{
    ...
    if (nHeight < 2)
    {
        ...
        //00000000008fffffffffffffffffffffffffffffffffffffffffffffffffffff
	...
	if(nHeight >= 2 && nHeight < 2000){
		arith_uint256 bnLimitWithSteps;
		arith_uint256 nNewProofOfWorkLimit = arith_uint256("000000ffffffffffffffffffffffffffffffffffffffffffffffffffffffffff");
		//nProofOfWorkLimit = nNewProofOfWorkLimit.GetCompact();
		arith_uint256 nMaxLimit = arith_uint256("00ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff");
		arith_uint256 nStep = (nMaxLimit - nNewProofOfWorkLimit) / 2000;
		bnLimitWithSteps = nMaxLimit - (nStep * nHeight);
		return bnLimitWithSteps.GetCompact();
	}
