...
const char * const BITCOIN_CONF_FILENAME = "qubnet.conf";
const char * const BITCOIN_PID_FILENAME = "qubnetd.pid";
const char * const DEFAULT_DEBUGLOGFILE = "debug.log";

bool fQubnetMode = true;
bool fQubnetWallet = false;
ArgsManager gArgs;
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Qubnet
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Qubnet
    // Mac: ~/Library/Application Support/Qubnet
    // Unix: ~/.qubnet
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Qubnet";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Qubnet";
#else
    // Unix
    return pathRet / ".qubnet";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good())
        return; // No qubnet.conf file is OK
