...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") || gArgs.IsArgSet("-help"))
    {
        ...
        std::string strUsage = strprintf(_("%s qubnet-tx utility version"), _(PACKAGE_NAME)) + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              "  qubnet-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded qubnet transaction") + "\n" +
              "  qubnet-tx [options] -create [commands]   " + _("Create hex-encoded qubnet transaction") + "\n" +
              "\n";
...
static void MutateTxVersion(CMutableTransaction& tx, const std::string& cmdVal)
{
    ...
    if (newVersion < 1 || newVersion > CTransaction::MAX_STANDARD_QUBNET_VERSION)
        throw std::runtime_error("Invalid TX version requested");

    if (!tx.IsQubnetVersion() && IsQubnetTxVersion(newVersion))
    {
...
static void MutateTxSign(CMutableTransaction& tx, const std::string& flagStr)
{
    ...
    for (unsigned int i = 0; i < mergedTx.vin.size(); i++) {
        ...
        if (tx.IsQubnetVersion() && amount == 0)
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        tx.nVersion = CTransaction::CURRENT_QUBNET_VERSION;
        int startArg;
