...
ALL_SCRIPTS = EXTENDED_SCRIPTS + BASE_SCRIPTS + QUBNET_SCRIPTS + INSIGHT_SCRIPTS
...
def main():
    ...
    parser.add_argument('--qubnet', action='store_true', help='run only Qubnet specific tests')
    ...
    tmpdir = "%s/qubnet_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    run_tests(test_list, config["environment"]["SRCDIR"], config["environment"]["BUILDDIR"], config["environment"]["EXEEXT"], tmpdir, args.jobs, args.coverage, passon_args, args.combinedlogslen, create_cache=(True if not args.qubnet and not args.insight else False))
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0, create_cache=True):
    ...
    try:
        if subprocess.check_output(["pidof", "qubnetd"]) is not None:
            print("%sWARNING!%s There is already a qubnetd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    if "BITCOIND" not in os.environ:
        os.environ["BITCOIND"] = build_dir + '/src/qubnetd' + exeext
        os.environ["BITCOINCLI"] = build_dir + '/src/qubnet-cli' + exeext
