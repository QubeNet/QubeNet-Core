...
UniValue getrawtransaction(const JSONRPCRequest& request)
{
    ...
    if (!fQubnetMode && hash == Params().GenesisBlock().hashMerkleRoot) {
...
UniValue createrawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx     (numeric or string, required) The key is the qubnet address, the numeric value (can be string) is the " + CURRENCY_UNIT + " amount\n"
    ...
    rawTx.nVersion = fQubnetMode ? QUBNET_TXN_VERSION : BTC_TXN_VERSION;
...
UniValue decoderawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "         \"type\" : \"pubkeyhash\",  (string) The type, eg 'pubkeyhash'\n"
            "         \"addresses\" : [           (json array of string)\n"
            "           \"PfqK97PXYfqRFtdYcZw82x3dzPrZbEAcYa\"   (string) qubnet address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
